...
from omix.trainer_omix_t import (
...
hyperparameter_defaults = dict(
    task = 'multiomic',
    seed=42,
    dataset_name="omix_t_pretrain", # Dataset name
    ...
    batch_size=4, # Default batch size for fine-tuning
    layer_size=512,
    nlayers=6,
    ...
    num_routers=1,
    top_k=4,
    modality_dict={'RNA':0, 'Protein': 1, 'METHYL':2, 'TEXT':3},
    num_modalities=4,
    # uniformity_weight=20,
    # gate_loss_weight=1,
    ...
    frozen_encoder=True,
    use_LoRA=True,
    text_only_lora=True, # True也就是encoder完全frozen，False则是全部encoder都LoRA
    # lora_rank=8,
    # lora_alpha=16,
    lora_rank=16,
    lora_alpha=32,
...
# --- 分布式训练设置 ---
dist.init_process_group(backend="nccl",timeout=datetime.timedelta(minutes=30)) # 使用 NCCL 后端，GPU通信效率高
...
# file_path = './OMedical/qwen_data_layer_20_maxlen512.h5'
# file_path = './OMedical/qwen_data_layer_20_maxlen1024_mutation.h5'
# file_path = './OMedical/tcga_processed_data.h5'
# file_path = './OMedical/tcga_processed_data_hunyuan.h5'
# file_path = '../../data/pretraining_data/generated_files/textual_annotations_deepseek_final.json'
# file_path = '../../data/pretraining_data/generated_files/textual_annotations_llama_final.json'
file_path = '../../data/pretraining_data/generated_files/textual_annotations_llama_clean.json'

with open(file_path, 'r') as f:
    text_data_preview = json.load(f)
text_patient_ids = list(text_data_preview.keys())

# all_ptid = list(set(adata.obs.index.tolist() + 
...
if config.load_model_Protein is not None:
    ...
    # 注意：这里变量名要统一，下面用的是 proteins_ids_Protein 还是 proteins_ids？
    # 建议统一命名，这里暂存为 proteins_ids_Protein
...
encoder_dict['TEXT'] = TrainableTextEncoder(
    model_name_or_path=youtu_model_path,
    output_dim=config.layer_size,
...
encoder_dict['TEXT'].to(device)


if config.use_LoRA:
    if rank == 0:
        logger.info("Applying LoRA wrappers...")
        if config.text_only_lora:
            logger.info("Mode: Text-Only LoRA. Omics encoders will be frozen (no LoRA), decoders trainable.")

    decoder_keywords = ["cls_decoder", "decoder", "mvc_decoder", "decode_head"]

    for mod, encoder in encoder_dict.items():
        # =========================================================
        # 情况 1: TEXT 模态 (始终应用 LoRA)
        # =========================================================
        if mod == 'TEXT':
            if rank == 0: logger.info(f"Configuring LoRA for {mod} (Youtu/Llama architecture)...")
            ...
            # 【修复点】在分支内部直接应用模型，避免跳出后变量未定义
            peft_model = get_peft_model(encoder, lora_config)
            if rank == 0:
                print(f"[{mod}] LoRA applied.")
                peft_model.print_trainable_parameters()
            
            encoder_dict[mod] = peft_model

        # =========================================================
        # 情况 2: Omics 模态 (RNA, Protein, METHYL)
        # =========================================================
        else:
            # Performer 固定逻辑 (保持不变)
            if hasattr(encoder, "transformer_encoder"):
                 performer = encoder.transformer_encoder
                 if hasattr(performer, "feature_redraw_interval"):
                     performer.feature_redraw_interval = None
                 if hasattr(performer, "auto_check_redraw"):
                     performer.auto_check_redraw = False
                 if hasattr(performer, "fix_projection_matrices_"):
                     performer.fix_projection_matrices_()

            # --- 分支 A: Text Only LoRA (Omics 不加 LoRA) ---
            if config.text_only_lora:
                if rank == 0: logger.info(f"[{mod}] explicit freeze (No LoRA), unfreezing decoder only.")
                ...
                for name, param in encoder.named_parameters():
                    if any(keyword in name for keyword in decoder_keywords):
                        param.requires_grad = True
                
                # 【关键】直接赋值原始 encoder，不经过 get_peft_model
                encoder_dict[mod] = encoder

            # --- 分支 B: 全模态 LoRA ---
            else:
                if rank == 0: logger.info(f"Configuring LoRA for {mod} (Performer architecture)...")
    ...
    if config.frozen_encoder:
        ...
        for modality, encoder in encoder_dict.items():
            if modality == 'TEXT':
                if rank == 0:
                    logger.info(f"Encoder for {modality} will remain trainable.")
                for param in encoder.parameters():
                    param.requires_grad = True # 确保文本编码器是可训练的
            else:
                # ... 原有的冻结逻辑 ...
                for name, param in encoder.named_parameters():
...
vocab_mod.update({'<pad>': 4})

# 或者一行代码
vocab_mod = {**config.modality_dict, '<pad>': 4}
...
if config.frozen_encoder:
    ...
    for modality, encoder in encoder_dict.items():
        ...
        if rank == 0:  # 只在主进程打印
            logger.info(f'{modality} Encoder using for finetuning')
            for name, param in encoder.named_parameters():
                if param.requires_grad:
                    logger.info(f"  - {name}")
    if rank == 0:  # 只在主进程打印
            logger.info(f'fusion_model using for finetuning')
...
omics_gene_ids = {
    'RNA': gene_ids_RNA,
    'Protein': proteins_ids_Protein,   # 注意：你的代码前面定义的是 proteins_ids
...
# 1. 【新增】初始化 Momentum Encoder
# 深度拷贝 encoder_dict，并冻结参数
momentum_encoder_dict = {}
for key, module in encoder_dict.items():
    # 注意：这里 module 是 DDP 包装过的，我们要 copy module.module (原始模型)
    # 或者直接 copy DDP 也可以，但通常 copy 原始模型更干净
    base_model = module.module if isinstance(module, DDP) else module
    m_encoder = copy.deepcopy(base_model)
    m_encoder.to(device)
    m_encoder.train() # 保持 BN 更新
    for param in m_encoder.parameters():
        param.requires_grad = False
    momentum_encoder_dict[key] = m_encoder
base_fusion = fusion_model.module if isinstance(fusion_model, DDP) else fusion_model
...
for epoch in range(1, config.epochs + 1):
    ...
    #     if rank == 0:
    #         logger.info(f"New best model found at epoch {best_model_epoch} with validation loss {best_val_loss:.4f}")
    #         logger.info(f"Saving model to {save_dir}")
