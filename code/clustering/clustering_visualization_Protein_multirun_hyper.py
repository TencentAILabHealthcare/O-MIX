# %%
"""
Hyper (multi-checkpoint) Protein (RPPA) clustering & visualization.

For a single pretrained run, sweep over a list of training-epoch checkpoints
(e.g. epochs 1 / 3 / 5 / 7 / 9) and, for each checkpoint, run the downstream
...
def run_single_clustering(adata: anndata.AnnData, embeddings: np.ndarray, seed: int,
    ...
    fig = sc.pl.tsne(
        ...
        title=f't-SNE of Protein Embeddings by Cancer Type (epoch={epoch}, seed={seed})',
        show=False,
        return_fig=True,
    )
    fig_path = fig_dir / f'embeddings_protein_pretrain{load_model}_e{epoch}_seed{seed}.png'
...
def main():
    ...
    csv_path = FIG_DIR / 'multirun_protein_hyper_metrics.csv'
    results_df.to_csv(csv_path, index=False)

    summary_csv_path = FIG_DIR / 'multirun_protein_hyper_metrics_summary.csv'
    summary_df.to_csv(summary_csv_path, index=False)

    summary_txt_path = FIG_DIR / 'multirun_protein_hyper_metrics_summary.txt'
