# %%
"""
Hyper (multi-checkpoint) Methylation clustering & visualization.

For a single pretrained run, sweep over a list of training-epoch checkpoints
(e.g. epochs 20 / 25 / 30 / 35 / 40) and, for each checkpoint, run the
downstream clustering / visualization pipeline with multiple random seeds.
ARI / NMI / Silhouette are reported as mean ± std per epoch.
...
def load_and_combine_data(disease_list: list, data_dir: str) -> anndata.AnnData:
    """Load multiple cancer datasets (methylation), tag each with its pancancer type, and concatenate."""
...
def run_single_clustering(adata: anndata.AnnData, embeddings: np.ndarray, seed: int,
    ...
    fig = sc.pl.tsne(
        ...
        title=f't-SNE of Methylation Embeddings by Cancer Type (epoch={epoch}, seed={seed})',
        show=False,
        return_fig=True,
    )
    fig_path = fig_dir / f'embeddings_methylation_pretrain{load_model}_e{epoch}_seed{seed}.png'
...
def main():
    ...
    csv_path = FIG_DIR / 'multirun_methylation_hyper_metrics.csv'
    results_df.to_csv(csv_path, index=False)

    summary_csv_path = FIG_DIR / 'multirun_methylation_hyper_metrics_summary.csv'
    summary_df.to_csv(summary_csv_path, index=False)

    summary_txt_path = FIG_DIR / 'multirun_methylation_hyper_metrics_summary.txt'
