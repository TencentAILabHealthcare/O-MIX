# %%
"""
Multi-run methylation clustering & visualization.

Runs the downstream clustering/visualization pipeline 5 times with different
random seeds on the SAME set of pretrained methylation embeddings (model
inference is deterministic in eval mode), and reports ARI / NMI as mean ± std.
...
def get_pancancer_embeddings(model: PerformerModel, adata: anndata.AnnData, vocab: GeneVocab) -> np.ndarray:
    """Use the pretrained methylation model to produce a sample-level embedding for every row of adata."""
...
def run_single_clustering(adata: anndata.AnnData, embeddings: np.ndarray, seed: int,
    ...
    fig = sc.pl.tsne(
        ...
        title=f't-SNE of Methylation Embeddings by Cancer Type (seed={seed})',
        show=False,
        return_fig=True,
    )
    fig_path = fig_dir / f'embeddings_methylation_pretrain{load_model}_seed{seed}.png'
...
def main():
    ...
    csv_path = FIG_DIR / 'multirun_methylation_metrics.csv'
    results_df.to_csv(csv_path, index=False)

    summary_path = FIG_DIR / 'multirun_methylation_metrics_summary.txt'
