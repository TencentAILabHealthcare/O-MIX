...
from omix.preprocess_rppa import Preprocessor  # 假设您的预处理类在这里
...
BATCH_SIZE = 64  # 推理时可以设置大一点的batch size
N_BINS = 51  # 必须与模型训练时使用的n_bins一致
PAD_TOKEN = "<pad>"
PAD_VALUE = N_BINS
MASK_VALUE = N_BINS+1
MAX_SEQ_LEN = 17007 + 1  # 必须与模型训练时一致 (n_hvg + 1)
...
def run_raw_data_analysis(adata_raw):
    ...
    raw_truth_file = 'tsne_raw_by_cancer_type_protein.png'
    fig_raw_truth.savefig(raw_truth_file, dpi=300, bbox_inches='tight')
    print(f"t-SNE plot saved to {raw_truth_file}")

    # 可视化2: 按Leiden聚类结果着色
    fig_raw_leiden = sc.pl.tsne(
        adata_raw,
        color='leiden_raw',
        title='t-SNE of Raw Data by Leiden Clusters (PCA-based)',
        legend_loc='on data',
        show=False,
        return_fig=True
    )
    raw_leiden_file = 'tsne_raw_by_leiden_cluster_protein.png'
    fig_raw_leiden.savefig(raw_leiden_file, dpi=300, bbox_inches='tight')
    print(f"t-SNE plot saved to {raw_leiden_file}")
...
def get_pancancer_embeddings(model: PerformerModel, adata: anndata.AnnData, vocab: GeneVocab) -> np.ndarray:
    ...
    cc = np.array(all_values)

    # adata_raw = anndata.AnnData(X=np.array(all_values), obs=pd.DataFrame(adata.obs['pancancer_type']))
    # run_raw_data_analysis(adata_raw)
...
def main():
    ...
    # model_file = model_dir / "model_e3.pt"
    ...
    embedding_file = f'embeddings_protein_pretrain{load_model}.png'
    ...
    #     return_fig=True
    # )
    # leiden_file = f'tsne_by_leiden_cluster_pretrain{load_model}.png'
