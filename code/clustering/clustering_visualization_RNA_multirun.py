# %%
"""
Multi-run RNA clustering & visualization.

Runs the downstream clustering/visualization pipeline 5 times with different
random seeds on the SAME set of pretrained RNA embeddings (model inference is
deterministic in eval mode), and reports ARI / NMI as mean ± std.
...
def get_pancancer_embeddings(model: PerformerModel, adata: anndata.AnnData, vocab: GeneVocab) -> np.ndarray:
    """Use the pretrained model to produce a sample-level embedding for every row of adata."""
...
def run_single_clustering(adata: anndata.AnnData, embeddings: np.ndarray, seed: int,
    ...
    # Work on a shallow copy so each seed is independent.
    ...
    fig = sc.pl.tsne(
        ...
        title=f't-SNE of RNA Embeddings by Cancer Type (seed={seed})',
        show=False,
        return_fig=True,
    )
    fig_path = fig_dir / f'embeddings_rna_pretrain{load_model}_seed{seed}.png'
...
def main():
    ...
    csv_path = FIG_DIR / 'multirun_rna_metrics.csv'
    results_df.to_csv(csv_path, index=False)

    summary_path = FIG_DIR / 'multirun_rna_metrics_summary.txt'
