...
class Preprocessor:
    ...
    def __init__(
        ...
        repair: bool=True,
        filter_gene_by_variance: Union[float, bool] = False,
    ):
        r"""
        ...
        self.repair = repair
        self.filter_gene_by_variance = filter_gene_by_variance
    ...
    def __call__(self, adata: AnnData, batch_key: Optional[str] = None) -> Dict:
        ...
        if isinstance(self.filter_gene_by_variance, (int, float)) and self.filter_gene_by_variance > 0:
            variance_threshold = self.filter_gene_by_variance
            logger.info(f"Filtering genes by variance > {variance_threshold} ...")

            # 从刚刚经过log1p处理的层获取数据
            data_layer = _get_obs_rep(adata, layer=key_to_process)

            # 如果是稀疏矩阵，需要转换为密集矩阵来计算方差
            if issparse(data_layer):
                data_layer_dense = data_layer.toarray()
            else:
                data_layer_dense = data_layer

            # 计算每个基因的方差 (沿 axis=0, 即跨越所有细胞)
            variances = np.var(data_layer_dense, axis=0)

            # 创建一个布尔掩码以保留符合条件的基因
            genes_to_keep = variances > variance_threshold

            n_genes_before = adata.n_vars

            # 应用筛选，这个方法会直接修改adata对象
            adata._inplace_subset_var(genes_to_keep)

            n_genes_after = adata.n_vars
            logger.info(f"    Filtered out {n_genes_before - n_genes_after} genes. "
                        f"Kept {n_genes_after} genes with variance > {variance_threshold}.")
            print(f'step 4.5: filter genes by variance > {variance_threshold}')

        if self.repair:
            # --- 开始：添加的修复代码 ---
            self.hvg_use_key = key_to_process
            hvg_layer_key = self.hvg_use_key

            # 从adata中获取HVG计算所需的数据层
            if hvg_layer_key is None:
                hvg_data = adata.X
            else:
                hvg_data = adata.layers[hvg_layer_key]

            if np.issubdtype(hvg_data.dtype, np.integer):
                logger.warning(
                    f"Data for HVG selection is of integer type. Casting to float32."
                )
                hvg_data = hvg_data.astype(np.float32)

            clip_threshold = 15.0
            n_clipped = np.sum(hvg_data > clip_threshold)
            if n_clipped > 0:
                logger.warning(
                    f"Clipping {n_clipped} values greater than {clip_threshold} in HVG input "
                    f"data to prevent overflow."
                )
                # 如果是稀疏矩阵，需要特殊处理
                if hasattr(hvg_data, "data"):
                    hvg_data.data[hvg_data.data > clip_threshold] = clip_threshold
                else:  # 如果是密集矩阵
                    hvg_data[hvg_data > clip_threshold] = clip_threshold

            if hvg_layer_key is None:
                adata.X = hvg_data
            else:
                adata.layers[hvg_layer_key] = hvg_data
            # --- 结束：添加的修复代码 ---
        ...
        if self.binning:
            ...
            for row in layer_data:
                ...
                # if isinstance(non_zero_ids, tuple):
                #     non_zero_ids = non_zero_ids[0]
