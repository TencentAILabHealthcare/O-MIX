...
def create_flexmoe_input(all_ptid, modalities_data, config, rank, save_path=None):
    ...
    for mod_name, mod_data in modalities_data.items():
        ...
        if mod_name == 'TEXT':
            # --- 1. 初始化 Tokenizer ---
            model_path = mod_data.get('model_path', './omix/Youtu_embedding')
            json_path = mod_data['file_path']
            max_length = config.max_seq_len_text
            ...
            for global_idx, ptid in enumerate(all_ptid):
                if ptid in raw_text_data:
                    # 拼接：Instruction + Content
                    text_content = raw_text_data[ptid]
                    full_text = text_content
                    aligned_raw_texts.append(full_text)
                    valid_indices.append(global_idx)
                    
                    observed_idx_arr[global_idx, config.modality_dict[mod_name]] = True
                    update_modality_combinations(global_idx, mod_char)
                else:
                    aligned_raw_texts.append("") 

            # --- 4. 批量 Tokenize (修改版：支持本地缓存) ---
            print(f"Processing Text Data (Total: {len(aligned_raw_texts)})...")
            
            # 构造缓存文件路径：放在与 json_path 同级目录下
            # 文件名包含: 原始文件名 + max_len + 病人数量, 确保配置变动时不会误读旧缓存
            cache_dir = os.path.dirname(json_path)
            json_name = os.path.basename(json_path).replace('.json', '')
            cache_filename = f"cached_{json_name}_len{max_length}_pts{n_patients}.pt"
            cache_path = os.path.join(cache_dir, cache_filename)
            
            # 初始化全 0 Tensor
            all_input_ids = torch.zeros((n_patients, max_length), dtype=torch.long)
            all_attention_mask = torch.zeros((n_patients, max_length), dtype=torch.long)
            
            loaded_from_cache = False
            
            # === 尝试加载缓存 ===
            if os.path.exists(cache_path) and rank == 0: # 只有主进程打印日志，但所有进程都加载
                print(f"[Cache] Found cached text tokens at: {cache_path}")
            
            if os.path.exists(cache_path):
                try:
                    # map_location='cpu' 避免占用 GPU 显存
                    cached_data = torch.load(cache_path, map_location='cpu')
                    
                    # 校验形状是否匹配
                    if (cached_data['input_ids'].shape == (n_patients, max_length) and 
                        cached_data['attention_mask'].shape == (n_patients, max_length)):
                        
                        all_input_ids = cached_data['input_ids']
                        all_attention_mask = cached_data['attention_mask']
                        loaded_from_cache = True
                        if rank == 0: print("[Cache] Successfully loaded text tokens from disk.")
                    else:
                        if rank == 0: print("[Cache] Shape mismatch. Re-tokenizing...")
                except Exception as e:
                    if rank == 0: print(f"[Cache] Error loading cache: {e}. Re-tokenizing...")
            
            # === 如果没有缓存或加载失败，则执行 Tokenize ===
            if not loaded_from_cache:
                if rank == 0: print(f"Tokenizing {len(valid_indices)} valid texts (Batch Processing)...")
                
                if len(valid_indices) > 0:
                    texts_to_process = [aligned_raw_texts[i] for i in valid_indices]
                    
                    batch_size = 2000 
                    num_samples = len(texts_to_process)
                    
                    from tqdm import tqdm

                    # 只有 rank 0 显示进度条，避免多进程打印混乱
                    iterator = range(0, num_samples, batch_size)
                    if rank == 0:
                        iterator = tqdm(iterator, desc="Tokenizing batches")
                    
                    for i in iterator:
                        batch_texts = texts_to_process[i : i + batch_size]
                        batch_global_indices = valid_indices[i : i + batch_size]
                        ...
                        all_input_ids[batch_global_indices] = encoded_batch['input_ids']
                        all_attention_mask[batch_global_indices] = encoded_batch['attention_mask']
                
                # === 保存缓存 (只由 Rank 0 保存，避免写冲突) ===
                if rank == 0:
                    print(f"[Cache] Saving tokenized data to {cache_path}...")
                    try:
                        torch.save({
                ...
                # 等待主进程保存完毕 (可选，但在 DDP 初始化前通常不需要 barrier)

            # --- 5. 存入 data_dict ---
            data_dict['TEXT_input_ids'] = all_input_ids.numpy()
            data_dict['TEXT_attention_mask'] = all_attention_mask.numpy()
            
            text_info_dict['processed'] = True
        else:
            # === 【修改开始】 ===
...
class MultiModalDataset(Dataset):
    def __init__(self, data_dict, observed_idx, config, vocab_dict):
        ...
        # 3. TEXT
        if 'TEXT_input_ids' in data_dict:
            self.text_input_ids = data_dict['TEXT_input_ids'][sorted_internal_indices]
            self.text_attention_mask = data_dict['TEXT_attention_mask'][sorted_internal_indices]

        # 4. Omics
        self.omics_indices = {}
        self.omics_paths = {}
        self.omics_gene_ids = {} # 仍然需要传给 collate_fn
    ...
    def __getitem__(self, idx):
        ...
        # 1. TEXT (已经 Pad 好了，直接取)
        if 'TEXT' in self.config.modality_dict and hasattr(self, 'text_input_ids'):
             sample_raw['TEXT_input_ids'] = self.text_input_ids[idx] # numpy
             sample_raw['TEXT_attention_mask'] = self.text_attention_mask[idx] # numpy
...
class DataCollator:
    ...
    def __call__(self, batch):
        ...
        # 1. TEXT 处理
        if 'TEXT' in self.config.modality_dict:
            # Stack numpy arrays -> Tensor
            if 'TEXT_input_ids' in raw_dicts[0]:
                collated_data['TEXT_input_ids'] = torch.tensor(np.stack([d['TEXT_input_ids'] for d in raw_dicts]), dtype=torch.long)
                collated_data['TEXT_attention_mask'] = torch.tensor(np.stack([d['TEXT_attention_mask'] for d in raw_dicts]), dtype=torch.long)
...
def train(
    ...
    Strategy: ALBEF-style Decoupling
    1. Teacher (Momentum): Sees Clean Data -> Generates Keys for MoCo.
    2. Student (Branch A): Sees Clean Data -> Generates Queries for CLIP/Uniformity.
    3. Student (Branch B): Sees Masked Data -> Generates Input for MLM/Reconstruction.
    """
    import wandb
    ...
    for batch, (batch_data, batch_targets, batch_mcs, batch_observed) in enumerate(loader):
        ...
        with torch.cuda.amp.autocast(enabled=config.amp):
            ...
            with torch.no_grad():
                for modality in modality_dict.keys():
                    if modality == 'TEXT': real_seq_len = 1
                    else: real_seq_len = batch_samples[f"{modality}_genes"].shape[1]
                    ...
                    if mask.sum() > 0: 
                        if modality == 'TEXT':
                            # Text 本身就是 clean 的
                            input_ids = batch_samples["TEXT_input_ids"][mask]
                            attention_mask = batch_samples["TEXT_attention_mask"][mask]
                            encoded_out_k, _ = momentum_encoder_dict[modality](
                                input_ids=input_ids, attention_mask=attention_mask
                            )
                        else:
                            # Omics: 使用 batch_targets (Clean Values)
                            input_genes = batch_samples[f"{modality}_genes"][mask]
                            input_values_clean = batch_targets[f"{modality}_targets"][mask] # <--- 关键：使用 Clean
            ...
            # ============================================================
            # 这一步是为了让 Student 学会 Clean 数据的对齐，避免被 Mask 噪声带偏
            for modality in modality_dict.keys():
                if modality == 'TEXT': real_seq_len = 1
                else: real_seq_len = batch_samples[f"{modality}_genes"].shape[1]
                ...
                if mask.sum() > 0:
                    if modality == 'TEXT':
                        input_ids = batch_samples["TEXT_input_ids"][mask]
                        attention_mask = batch_samples["TEXT_attention_mask"][mask]
                        encoded_output, _ = encoder_dict[modality](input_ids=input_ids, attention_mask=attention_mask)
                ...
                # 缺失模态补全 (Clean Pass 也需要补全，否则 DDP 可能不同步，虽然 SimCLR 不用补全部分)
                if (~mask).sum() > 0:
            ...
            for modality in modality_dict.keys():
                ...
                if mask.sum() > 0:
                    if modality == 'TEXT':
                        # Text 不做 mask，直接复用 Clean 的结果，省计算
                        # encoded_samples_masked[mask] = pre_fusion_map_clean[modality_dict[modality]][mask] # 这样不行，没过 adapter 前的才行
                        # 重新计算太慢，Text 本来就是 clean 的，直接拿 Phase 2 的结果
                        # 注意：pre_fusion_map_clean 已经过了 adapter，所以这里直接用
                        encoded_samples_masked[mask] = pre_fusion_map_clean[modality_dict[modality]][mask]
                    else:
                        # Omics: 使用 batch_samples (Masked Values)
                        input_genes = batch_samples[f"{modality}_genes"][mask]
                        input_values_masked = batch_samples[f"{modality}_values"][mask] # <--- 关键：使用 Masked
                        src_key_padding_mask = input_genes.eq(vocab_dict[modality][config.pad_token])
                ...
                if (~mask).sum() > 0:
                    ...
                    missing_part = correct_missing_embeds_bank[batch_mcs[~mask], modality_dict[modality]]
    
                    # 【修复】：如果是 TEXT，手动让 Missing Part 过一次 Adapter
                    if modality == 'TEXT':
                        # 注意：这里需要 unsqueeze/reshape 来匹配 Adapter 的输入要求，如果 Adapter 接受 (B, L, D) 或 (N, D)
                        # 假设 run_adapter 可以处理 (N, D) 或者你需要 unsqueeze 成 (N, 1, D) 再 squeeze 回来
                        # 还要注意 DDP 的情况
                        fusion_base = fusion_model.module if isinstance(fusion_model, DDP) else fusion_model
                        missing_part = fusion_base.run_adapter(modality, missing_part)

                    # 赋值回 Tensor
                    encoded_samples_masked[~mask] = missing_part
                
                # Adapter
                if modality != 'TEXT': # Text 已经过了一次 Adapter (复用 Clean)，Omics 需要过
                     if isinstance(fusion_model, DDP):
                ...
                # 准备 Fusion Model 的输入 (这些是 Masked 的)
            ...
            for mod_name, mod_idx in config.modality_dict.items():
                ...
                if mask.sum() > 1:
                    raw_h = pre_fusion_map_clean[mod_idx] # 使用 Clean Feature
                    valid_h = raw_h[mask]
            ...
            # --- 4.2 Fusion Forward & SimCLR (使用 Clean Maps 计算 SimCLR，Masked Input 计算 MLM) ---
            
            # 为了防止 DDP 报错 (Unused parameters)，我们需要确保 dummy loss 覆盖了所有
            # 尤其是当某个 Batch 缺失某些模态时
            
            mod_types = torch.cat(mod_types_list, dim=1)
            
            # 【核心调用】
            # inputs: fusion_input_cls_masked (Masked, 用于 MLM 内部交互)
            # pre_fusion_map: pre_fusion_map_clean (Clean, 用于计算 SimCLR Loss)
            # pre_fusion_map_k: pre_fusion_map_k (Clean, 用于 SimCLR Target)
            ...
            # --- 4.3 MLM Loss (Gep/Gepc) 使用 Masked Input 的输出 ---
            tmp_gepc_loss = 0.0
            tmp_gep_loss = 0.0

            for index, modality in enumerate(modality_dict_keys):
                if modality == 'TEXT': continue # TEXT 没有重建

                present_mask = batch_observed[:, config.modality_dict[modality]]
                if not present_mask.any():
                    # Dummy Loss 防止 DDP 报错
                    current_enc = encoder_dict[modality].module if isinstance(encoder_dict[modality], DDP) else encoder_dict[modality]
                    dummy_loss_all = sum(p.sum() for p in current_enc.parameters()) * 0.0
                    list_of_all_losses.append(dummy_loss_all)
                else:
                    # 获取 Masked Input 对应的 Fusion Output
        ...
        if batch % log_interval == 0 and batch > 0:
            ...
            if rank == 0:
                ...
                # 这里需要手动计算 Clean Map 的两两相似度用于 Log (可选，或者直接用 SimCLR stats)
                # ... (原有 Sim Stats Logging 逻辑可以保留)
...
def evaluate(
    ...
    Strictly decoupled: 
    - Clean Pass -> CLIP metrics
    - Masked Pass -> MLM metrics
    """
    import wandb
    modality_dict = config.modality_dict
            ...
            with torch.cuda.amp.autocast(enabled=config.amp):
                ...
                for modality in modality_dict.keys():
                    ...
                    if modality == 'TEXT':
                        ...
                        input_genes = batch_samples[f"{modality}_genes"][mask]
                        # 【关键】使用 Clean Targets 作为 Input Values
                ...
                for modality in modality_dict.keys():
                    ...
                    if mask.sum() > 0:
                        if modality == 'TEXT':
                            # Text 没有 Mask，直接复用 Clean Pass 的结果，节省计算
                            encoded_samples_masked[mask] = pre_fusion_map_clean[modality_dict[modality]][mask]
                        else:
                            # 【关键】使用 Masked Samples
                            input_genes = batch_samples[f"{modality}_genes"][mask]
                    ...
                    if (~mask).sum() > 0:
                        correct_missing_embeds_bank = missing_embeds_dict[modality]
                        # 1. 先取出 Raw Missing Embeddings
                        missing_part = correct_missing_embeds_bank[batch_mcs[~mask], modality_dict[modality]]
                        
                        # 2. 如果是 TEXT，手动过一次 Adapter
                        if modality == 'TEXT':
                            missing_part = unwrapped_fusion_model.run_adapter(modality, missing_part)

                        # 3. 填入 Tensor
                        encoded_samples_masked[~mask] = missing_part
                    
                    # Adapter (Text 已经复用了 Clean 结果不用再跑，Omics 需要跑)
                    if modality != 'TEXT':
                ...
                # =========================================================
                # Metrics 2: CLIP Similarity Statistics (Based on Clean)
                # =========================================================
                batch_z_map = {}
                ...
                # =========================================================
                # Metrics 3: OmniFusionBlock Forward & Loss
                # =========================================================
                mod_types = torch.cat(mod_types_list, dim=1)
                
                # 注意：
                # 1. fusion_input_cls_masked -> 用于 MLM 交互 (Masked)
                # 2. pre_fusion_map_clean -> 用于 SimCLR 计算 (Clean)
                # 3. pre_fusion_map_k -> 在 Validation 阶段，我们通常希望看 "In-Batch" 对齐，
                #    或者 "Validation Set vs Training Queue" 的对齐。
                #    最简单的做法是把 pre_fusion_map_clean 传给 K，计算 In-Batch SimCLR Loss。
                ...
                hidden_x, simclr_loss_val, batch_sim_stats = unwrapped_fusion_model(
                    *fusion_input_cls_masked, 
                    ...
                    pre_fusion_map_k=pre_fusion_map_clean,  # Key (Clean - 自对比)
                    modality_pairs=modality_pairs,
                    text_loss_weight=config.text_loss_weight,
                    bio_loss_weight=config.bio_loss_weight,
