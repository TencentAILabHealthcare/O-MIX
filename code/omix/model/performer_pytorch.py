...
from omix.performer_pytorch.reversible import ReversibleSequence, SequentialSequence
...
#         # 将输入数据和投影矩阵临时转换为float32
#         data_float = data.float()
#         projection_float = projection.float()
#
#         data_dash = torch.einsum('...id,...jd->...ij', (data_normalizer * data_float), projection_float)
#         diag_data = data_float ** 2
#         diag_data = torch.sum(diag_data, dim=-1)
#         diag_data = (diag_data / 2.0) * (data_normalizer ** 2)
#         diag_data = diag_data.unsqueeze(dim=-1)
#
#         if is_query:
#             max_val = torch.max(data_dash, dim=-1, keepdim=True).values
#             data_dash = ratio * (
#                 torch.exp(data_dash - diag_data - max_val) + eps)
#         else:
#             max_val = torch.max(data_dash)
#             data_dash = ratio * (
#                 torch.exp(data_dash - diag_data - max_val) + eps)
#
...
class SelfAttention(nn.Module):
    ...
    def forward(self, x, pos_emb = None, context = None, mask = None, context_mask = None, output_attentions = False, **kwargs):
        ...
        if not empty(q):
            ...
            if output_attentions:
                out, (q_prime, k_prime) = self.fast_attention(q, k, v, output_attentions)
                attn_factors_list.append((q_prime, k_prime))
            else:
        ...
        out =  self.to_out(out)
        if output_attentions:
            if len(attn_factors_list) > 0:
                return self.dropout(out), attn_factors_list[0]
            else:
...
class PerformerLM(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.norm = nn.LayerNorm(dim)
        self.cls_decoder = ClsDecoder(dim, 1)
