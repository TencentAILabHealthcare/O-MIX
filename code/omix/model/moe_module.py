...
import tree
from fmoe.functions import AllGather, Slice, ensure_comm
...
class FMoETransformerMLP(FixedFMoE):
    ...
    * `d_hidden` is the dimension of the MLP layer.
    ...
    def forward(self, inp: torch.Tensor, expert_indices=None):
        ...
        normalization.
        """
        assert inp.device.index == self.experts.h4toh.weight.device.index, "Input and expert weights are on different devices!"
        if expert_indices is not None:
            assert expert_indices.device == inp.device, "Input and expert_indices are on different devices!"
...
class AddtionalNoisyGate(NoisyGate):
    ...
    def get_loss_clear(self, clear=True):
        """获取并可选地清理loss"""
        # print('clear')
    ...
    def forward(self, inp, expert_indices=None):
        ...
        if (expert_indices != None) & (expert_indices.sum() > 0):
            # print('fsadasdsaasdfa')
            # print(expert_indices)
            # print(expert_indices.sum())
        ...
        if (expert_indices != None):
            ...
            if full_modality_mask.sum() == len(full_modality_mask):
                ...
                loss = loss + self.cv_squared(importance) + self.cv_squared(load)
            else:
