...
class SequentialSequence(nn.Module):
    ...
    def forward(self, x, output_attentions = False, **kwargs):
        ...
        attn_factors_list = [] # 用于存储每一层的 (q', k')
        ...
        for (f, g), (f_args, g_args) in layers_and_args:
            if output_attentions:
                ...
                out, attn_factors = f(x, output_attentions=True, **f_args)
                
                # attn_factors 是 tuple (q_prime, k_prime)
                # 直接存入列表，不要 unsqueeze，也不要 cat，否则维度对不上
                attn_factors_list.append(attn_factors)
                x = x + out
            else:
                x = x + f(x, **f_args)
            x = x + g(x, **g_args)
        if output_attentions:
            return x, attn_factors_list
        else:
            return x
